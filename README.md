...
- **아름다운재단** (`change.beautifulfund.org`): WordPress REST API로 "청년"/"고립"/"은둔" 검색
- **청년지원기관 목록 (수동 등록, [scraper/sources-foundations.js](scraper/sources-foundations.js))**: 종사자가 직접 조사한 약 30개 청년지원기관 홈페이지를 고정 리스트로 크롤링. Google Custom Search를 통한 자동 발견이 결제 계정 미연동으로 막혀 있어, 현재는 이 수동 목록이 재단·기관 소스의 주력
- Google Custom Search API로 "고립청년 재단" 검색 → 도메인 자동 추출, **주 1회** 갱신해 `docs/data/foundations.json`에 캐싱하는 자동 발견 로직도 남겨뒀지만 현재는 작동하지 않음
- 좌측 7개 단체 홈페이지(있는 경우)에서도 공모/지원사업 게시물 크롤링
- 모든 소스는 "고립"/"은둔"/"청년"/"청소년" 중 하나라도 언급된 것만 통과 — 사회연대은행·신한금융희망재단처럼 청년 외 다양한 대상을 함께 다루는 기관에서 무관한 항목(소상공인 지원, 위기가정 지원 등)이 섞이는 것을 막기 위함
- "[마감]"/"모집마감"/"결과발표"/"최종선정" 등의 문구나 제목에 박힌 마감일이 지난 경우는 "closed"로 분류해 우측 하단 "마감된 공모사업"에 표시 (삭제하지 않음)
